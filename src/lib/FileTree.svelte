<script lang="ts">
  ...
  import { projectRoot, hiddenPatterns } from './stores.ts';
    ...
    <div class="tree-header">
      ...
      <div class="tree-header-actions">
        <button class="header-btn" title="New file" onclick={() => startCreate('file')}>
          <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" width="13" height="13">
        ...
        <button class="header-btn" title="Open folder" onclick={openFolder}>
          <svg viewBox={icons.folderPlus.viewBox} fill="currentColor" width="13" height="13">
            <path d={icons.folderPlus.path} />
          </svg>
        </button>
        <button class="header-btn" title="Refresh" onclick={() => loadDirectory(rootPath!)}>
          ↻
        </button>
      </div>
    </div>
...
{#snippet fileNode(entry: FileEntry, depth: number)}
  ...
  >
    ...
    {/if}
    <span class="file-name" class:dir-name={entry.is_dir}>{entry.name}</span>
  </button>
